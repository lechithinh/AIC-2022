...
#config paths
IMAGE_KEYFRAME_PATH = r"/home/ngohiep/Documents/AI_Challenge/Keyframes"
VISUAL_FEATURES_PATH = r"/home/ngohiep/Documents/AI_Challenge/CLIPFeatures"
KEYFRAME_P_PATH = r'home/ngohiep/Documents/AI_Challenge/Keyframe_p/keyframe_p'
META_LINK = r'/home/ngohiep/Documents/AI_Challenge/metadata'
...
class Filter():
    def __init__(self):
        f = open("keyframe_files.json", "r")
        self.keyframe_files = json.load(f) #
        f = open('transcripts.json', 'r')
        ...
        for key in keys:
            ...
            for i in range(len(self.transcripts[key])):
                self.transcripts[key][i]['text'] = self.transcripts[key][i]['text'].lower()
                if old_index is not None:
                    self.transcripts[key][old_index]['text'] += ' ' + self.transcripts[key][i]['text']
                    self.transcripts[key][old_index]['duration'] += self.transcripts[key][i]['duration']
                self.transcripts[key][i]['estimated_keyframe'] = int(float(self.transcripts[key][i]['start']) * 24.0)
                old_index = i
        f = open('objects.json', 'r')
        self.objects = json.load(f) #

        f = open('reversed_objects.json', 'r')
    ...
    def find_set_of_nearest_frame(self,video_name, estimated_keyframe, left_bound=0, right_bound=1):
        if left_bound + right_bound <= 0:
            right_bound = 1
            
        keyframe_index = bisect_left(self.keyframe_files[video_name], estimated_keyframe)
        
        num_of_frame = len(self.keyframe_files[video_name])

        index = self.keyframe_files[video_name][keyframe_index:min(num_of_frame, keyframe_index+right_bound)] + \
                self.keyframe_files[video_name][max(0, keyframe_index-left_bound):keyframe_index]
        ...
        for key in self.transcripts.keys():
            for i in self.transcripts[key]:
                if keyword in i['text']:
                    ...
                    nearest_frame = self.find_set_of_nearest_frame(key, standardize_frameid(i['estimated_keyframe']), left_bound=0, right_bound=num_frame)
    ...
    def objects_filter(self,obj):
        ...
        res = set(self.reversed_objects[obj[0]]).intersection(self.reversed_objects[obj[1]])
...
class ImageEmbedding():
    ...
    def search_by_image(self,path_image: str, 
                    topk:int=5, 
                    measure_method: str="dot_product", 
                    filter_result=None) -> List[dict,]:
        ...
        if measure_method == 'cosine':
            ...
            for ins_id, instance in enumerate(self.visual_features_db):
                if filter_result is None or (self.visual_features_db[ins_id][0], self.visual_features_db[ins_id][1]) in filter_result:
...
class Searching():
    def __init__(self):
        ...
        self.TextModel = TextEmbedding(visual_features_db)
        self.ImageModel = ImageEmbedding(visual_features_db)
        self.FilterModel = Filter()
      

    def search(self, text_query = '', image_query = False, voice_filter_text = '',color_filter = '', objects_filter_text = '', top_k = 20, swap_top = ''):
        ...
        if len(objects_filter_text) > 0:
            objects_filter_result = self.FilterModel.objects_filter(objects_filter_text.split(','))
        ...
        if image_query:
            search_result = self.ImageModel.search_by_image(image_query, int(top_k), filter_result=filter_result)
            image_query = False
        else:
        
            search_result = self.TextModel.search_engine(text_query, int(top_k), filter_result=filter_result)
