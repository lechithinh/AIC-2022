...
class Searching():
    ...
    def search(self, text_query = '', image_query = False, drawing_query = '', voice_filter_text = '',color_filter = '', objects_filter_text = '', top_k = 20, swap_top = ''):
        ...
        if image_query:
            ...
            image_query = False
        elif drawing_query != '' and filter_result is not None: #điều kiện số lượng ảnh từ <5k
            search_result = sketch_engine(drawing_query, db= self.visual_features_db, topk=int(top_k), filter_result=filter_result)
        else:
